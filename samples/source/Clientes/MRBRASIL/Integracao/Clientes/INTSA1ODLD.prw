...
User Function SA1CSV() 
    ...
    Local _cQuery 
	local nHandle := FCREATE("\TRIMBOX\IN\CLIENTE.sql")
...
//fclose(nHandle)
  ...
  ConOut(_aArray[nt][1],_aArray[nt][2],_aArray[nt][3])
   ...
   FWrite(nHandle, "INSERT INTO cliente values ( '"+_aArray[nt][1]+"';'"+_aArray[nt][2]+"';'"+_aArray[nt][3]+"';'"+_aArray[nt][4]+"';'"+_aArray[nt][5]+"';'"+_aArray[nt][6]+"';'"+_aArray[nt][7]+"';'"+_aArray[nt][8]+"';'"+_aArray[nt][9]+"';'"+_aArray[nt][10]+"')"+ CRLF)
  ConOut("------------------------------------------")
  		Next
		ENDIF
		fclose(nHandle)
